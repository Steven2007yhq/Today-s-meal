...
产品官网：[hao-chi-de-jin-tian.steven108yhq.chatgpt.site](https://hao-chi-de-jin-tian.steven108yhq.chatgpt.site) · 用户说明：[README.pdf](public/docs/README.pdf)
...
- 中华菜品库：PostgreSQL 当前可导入 5000 道唯一菜品，支持菜名/食材/口味/做法检索、菜系/地区/类型筛选和服务端分页；每道菜只保存最多 24 条精选近邻，当前约 12 万条定向关系。客户端保留约 200 道离线基础库，后台不可用时仍能搜索和换算。
- 饭量引擎：综合用户同餐次的历史份量与单餐热量目标，自动换算菜品食材克数、热量和三大营养素；可录入一道菜中一种或多种食材的现有克数。单种食材直接按原配方缩放，两种以上使用可解释的相对加权最小二乘拟合，保留用户输入并补齐其余食材，同时显示拟合偏差；结果会进入餐单与食谱 PDF。
...
- 可读性与详情抽屉：正文和表单文本以 12px 为下限，辅助文字和焦点环提高对比度；菜品详情改为按需打开的侧边抽屉，支持遮罩、关闭按钮、Esc 退出和焦点返回，并通过 100%/125%/150% 缩放实测。
...
- 待发布：已生成与当前源码一致的 1.2.0 Windows NSIS 封闭测试包；当前产物未配置 Authenticode 代码签名，公开发布前仍需签名并完成新装、覆盖升级和卸载回归。
...
目录采用“完整服务端目录 + 小型离线基础库”结构，避免把 5000 条菜品、图片和完整关系图打进桌面包。服务端只返回当前页（默认 36、最多 60 条），前端只加载当前页图片；地区、菜品类型、来源、审核状态和营养可信度均是独立字段。

当前数据分三层：

- 项目已审阅配方：现有地方菜、节日和西式种子数据。
- 开源配方元数据：从 `Anduin2017/HowToCook` 的 Unlicense 仓库提取菜名、分类、原料和来源链接，不复制正文步骤或仓库图片。
- 产品自有家常搭配：用可解释的食材与做法规则补齐长尾，统一标为“家常搭配 / 组合规则生成 / 营养估算”，不冒充传统名菜或精确营养值。

构建与导入：

```powershell
# 可选：从本地 HowToCook 仓库重建开源配方快照
npm run catalog:source:howtocook -- "C:\path\to\HowToCook"

# 只生成目录和质量报告，不写数据库
npm run catalog:quality

# 迁移 PostgreSQL 并导入 5000 道菜、来源台账和 Top-24 关系
...
生成结果写入 `src/data/crawledDishLibrary.json`，应用构建时会与离线基础库自动合并；执行 `npm run catalog:seed` 时也会纳入服务端目录。外部来源必须先确认商业使用条件、署名要求和自动访问许可。
...
菜品没有专属图片时，前后端会共用同一套“主料 + 菜品形态 + 烹饪做法”规则，从现有图片中选择一张“同类示意图”，并在卡片与详情页明确标注来源菜名。专属本地图和专属云端图始终优先；主蛋白、内脏/普通肉、形态或做法冲突时宁可不显示图片。后台未启动时也可使用内置菜品图完成保守匹配；云端候选目录采用短时缓存并合并并发读取，上传或删除图片后会立即失效缓存。现有图片在正式上线前仍需完成授权审计。

类别基准图放在 `figure/category-reference-2026-08-25`，语义档案位于 `scripts/category_reference_manifest.json`。它们使用独立的 `category-ref-*` ID，不会冒充目录中的某一道精确菜；服务端只根据档案中的画面菜名、做法、形态和主料参与同类匹配。导入和覆盖审计均可重复执行：

```powershell
npm run images:category
npm run images:coverage
```

2026-08-25 本地数据库实测为 5000 道已发布菜、185 道专属图、3319 道保守同类图，总覆盖 3504 道（70.1%）；35 张类别基准图均已写入 PostgreSQL/MinIO。仍未安全匹配的菜会继续显示无图状态，不会为追求覆盖率而跨主蛋白、菜品形态或做法误配。

2026-08-26 第二批实际收到并导入 38 张类别基准图，累计 73 张。细分素鸡/鸡肉、刀削面/米线/河粉/米粉、杂粮包/普通包和麻辣香锅等形态冲突后，本地实测为 185 道专属图、3709 道保守同类图，总覆盖 3894 道（77.9%），仍有 1106 道保持无图。
